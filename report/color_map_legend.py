...
cb1 = fig.colorbar(plt.cm.ScalarMappable(cmap=vis_cmap), cax=axes[0], orientation='horizontal', label='Visibility')
cb1.set_ticks(np.linspace(0, 1, 11))
cb1.set_ticklabels([f"{i:.1f}" for i in np.linspace(0, 1, 11)])

# Color bar for angle color mapping
cb2 = fig.colorbar(plt.cm.ScalarMappable(cmap=angle_cmap), cax=axes[1], orientation='horizontal', label='Dot Product (cosine of angle)')
cb2.set_ticks(angle_boundaries)
cb2.set_ticklabels([">=90°", "75°-90°", "60°-75°", "45°-60°", "30°-45°", "15°-30°", "<15°"])
