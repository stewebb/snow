...
\subsection {Temperature and Snow Factor Simulation}
...
interpolation. The curve is hard to express by a regular triangular function as the time between the peak values is not equal. Besides,
there is no mapping between the location and the temperature. All the temperature data are manually defined so some problems may be 
caused if the location is set to most of the equator area. However, this may be an efficient method for simulating temperature in some
applications like video games or movies, as there are only a few scenes that need to be considered.
...
in the real world, the causing of snow is very complicated. However, this is still a simple approach to get started on the environment
simulation. 
